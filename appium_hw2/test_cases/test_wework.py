...
class TestWework:
    ...
    def test_add_member(self, name: str, gender, phone: str):
        if self.app.desired_caps['udid'] == '127.0.0.1:7555':
            name = f'{name}_{self.app.desired_caps["udid"]}'
            phone = phone.replace('xx', str(random.randint(10, 30)))
        elif self.app.desired_caps['udid'] == 'emulator-5554':
            name = f'{name}_{self.app.desired_caps["udid"]}'
            phone = phone.replace('xx', str(random.randint(31, 60)))
        else:
            phone = phone.replace('xx', str(random.randint(31, 60)))
